...
abstract class AbstractSignature(val messageDigest: AlgorithmInstance) : Signature {
  ...
  override fun init(key: Key.Private) {
    ...
    ctx = EVP_MD_CTX_new()
    EVP_DigestSignInit(ctx, null, messageDigest.ptr, null, pkey).checkTrue("EVP_DigestSignInit fails") {
      EVP_MD_CTX_free(ctx)
    }
    signMode = true
  }
  ...
  override fun init(key: Key.Public) {
    ...
    val pkey = createPkey(key)
    ctx = EVP_MD_CTX_new()
    if (EVP_DigestVerifyInit(ctx, null, messageDigest.ptr, null, pkey) != 1) {
      EVP_MD_CTX_free(ctx)
