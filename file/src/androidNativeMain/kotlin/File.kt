...
import platform.common.statvfs
...
actual class File actual constructor(path: String) {
  ...
  actual fun mkdir(): Boolean =
    mkdir(
      path,
      (S_IRUSR or S_IWUSR or S_IXUSR or S_IRGRP or S_IWGRP or S_IXGRP or S_IROTH or S_IWOTH or S_IXOTH).convert(),
    ) == 0
  ...
  actual fun getPosixMode(): PosixPermissions = memScoped {
    val stat = alloc<stat>()
    if (stat(path, stat.ptr) != 0) {
