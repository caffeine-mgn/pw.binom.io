...
//        freeze()
  ...
  actual fun getPosixMode(): PosixPermissions =
    memScoped {
      ...
      return PosixPermissions(stat.st_mode.convert())
    }
  ...
  actual fun relative(path: String): File =
    fileGetRelative(this, path)

  actual fun relative(path: Path): File =
    relative(path.raw)

  actual fun mkdirs(): File? =
    fileMkdirs(this)

  actual fun deleteRecursive(): Boolean =
    fileDeleteRecursive(this)
}
