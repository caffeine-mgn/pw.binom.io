...
data class DnsPackage(
  ...
  fun toImmutable() = DnsRecord(
    header = header,
    queries = question.map { it.toImmutable() },
    ans = answer.map { it.toImmutable() },
    auth = authority.map { it.toImmutable() },
    add = additional.map { it.toImmutable() },
  )
}
