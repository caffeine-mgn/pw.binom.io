...
class SelectorTest {
  ...
  fun connectOneShotSuccessTest() {
    ...
    key.updateListenFlags(ListenFlags.WRITE + ListenFlags.ONCE)
    client.connect(httpServerAddress)
    val events = tandem.select()
    assertEquals(1, events.size)
    assertSame(key, events.first().key)
    assertTrue(ListenFlags.ERROR !in events.first().flags, "flag not contains error flag")
    assertTrue(ListenFlags.WRITE in events.first().flags, "flag not contains write flag")
//        assertTrue(events.first().flags and KeyListenFlags.READ == 0, "flag not contains read flag")
    assertEquals(ListenFlags.ZERO, key.listenFlags)
  ...
  fun connectOneShotFailTest() {
    ...
    client.blocking = false
    val key = tandem.selector.attach(client)
    key.updateListenFlags(ListenFlags.WRITE + ListenFlags.ERROR + ListenFlags.ONCE)
    client.connect(DomainNetworkAddress("127.0.0.1").resolve()!!.withPort(1))
    println("#--1")
    val events = tandem.select()
    println("#--2")
    assertEquals(1, events.size)
    ...
    assertTrue(ListenFlags.ERROR !in events.first().flags)
    assertEquals(ListenFlags.ZERO, key.listenFlags)
    println("#--3")
