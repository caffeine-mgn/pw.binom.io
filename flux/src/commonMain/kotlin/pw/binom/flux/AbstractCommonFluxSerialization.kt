...
abstract class AbstractCommonFluxSerialization : FluxSerialization {
    ...
    override fun isBodySupported(mimeType: String): Boolean =
        getSerializationList().any { it.isBodySupported(mimeType) }
    ...
    override fun <T> decodeMap(serializer: KSerializer<T>, input: Map<String, String>): T {
        getSerializationList().forEach { flexSerializer ->
            if (!flexSerializer.isStringMapSupported()) {
                return@forEach
            }
    ...
    override suspend fun <T> encodeBody(mimeType: String, value: T, serializer: KSerializer<T>, output: AsyncOutput) {
        getSerializationList().forEach { flexSerializer ->
    ...
    override suspend fun <T> decodeBody(mimeType: String, serializer: KSerializer<T>, input: AsyncInput): T {
        getSerializationList().forEach { flexSerializer ->
            if (!flexSerializer.isBodySupported(mimeType)) {
                return@forEach
            }
    ...
    suspend fun <T> read(
        ...
        val type = exchange.requestHeaders.mimeType?.lowercase() ?: getDefaultMimeType()
            ?: throw FluxSerialization.SerializationNotSupported()
        type.split(',').forEach {
            val mime = it.trim()
    ...
    private suspend fun <T> response(
        ...
        val headerForSend = if (headers.isEmpty()) {
