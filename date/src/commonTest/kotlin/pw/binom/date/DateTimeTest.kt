package pw.binom.date

import kotlin.math.absoluteValue
import kotlin.test.Ignore
import kotlin.test.Test
import kotlin.test.assertEquals
import kotlin.test.assertTrue

class DateTimeTest {
  private val currentTimeZone = test_data_currentTZ
  private val now = test_data_now

  @Test
  fun timeZone() {
    assertEquals(currentTimeZone, DateTime.systemZoneOffset)
  }

  @Ignore
  @Test
  fun zeroTimeTest() {
    println("--->${DateTime(0).calendar().iso8601()}")
    val zeroTime = DateTime.of(year = 0, timeZoneOffset = 0).milliseconds
    assertEquals(TestData.ZERO_TIME, zeroTime)
    println("--------->${DateTime.of(year = 1960, timeZoneOffset = 0).milliseconds}")
    val c = DateTime.of(year = 1970, timeZoneOffset = 0).calendar()
    println("c=$c")
  }

  @Ignore
  @Test
  fun nowTest() {
    val except = now
    val actual = DateTime.nowTime
    assertTrue("except: [$except], actual: [$actual]") {
      (except - actual).absoluteValue < 10_000
    }
  }

  @Test
  fun ofTest() {
    val date =
      DateTime.of(
        year = 1989,
        month = 1,
        dayOfMonth = 5,
        hours = 13,
        minutes = 0,
        seconds = 0,
        millis = 0,
        timeZoneOffset = 0,
      )
    assertEquals(600008400000L, date.milliseconds)
  }

  @Test
  fun calendarTest() {
    val date =
      DateTime.of(
        year = 1989,
        month = 1,
        dayOfMonth = 5,
        hours = 13,
        minutes = 0,
        seconds = 0,
        millis = 0,
        timeZoneOffset = 0,
      ).calendar(0)

    assertEquals(1989, date.year, "year")
    assertEquals(1, date.month, "month")
    assertEquals(5, date.dayOfMonth, "dayOfMonth")
    assertEquals(13, date.hours, "hour")
    assertEquals(0, date.seconds, "second")
    assertEquals(0, date.millisecond, "millisecond")
  }

  @Test
  fun utcTest() {
    val d =
      DateTime.of(
        year = 2021,
        month = 6,
        dayOfMonth = 10,
        hours = 14,
        minutes = 0,
        timeZoneOffset = 0,
        seconds = 0,
        millis = 0,
      )
    assertEquals(1623333600000L, d.milliseconds)
    assertEquals("Thu, 10 Jun 2021 14:00:00 GMT", d.calendar().rfc822())
  }

  @Test
  fun mskTest() {
    val d =
      DateTime.of(
        year = 2021,
        month = 6,
        dayOfMonth = 10,
        hours = 14,
        minutes = 0,
        timeZoneOffset = 180,
        seconds = 0,
        millis = 0,
      )
    assertEquals(1623322800000L, d.milliseconds)
    assertEquals("Thu, 10 Jun 2021 11:00:00 GMT", d.calendar().rfc822())
  }
}
