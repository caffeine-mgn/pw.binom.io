...
actual class Calendar(private val utcTime: Long, actual val offset: Int) {
    ...
    init {
        val utcTime = utcTime + offset * DateMath.MILLISECONDS_IN_MINUTE
        var epochDay = (utcTime / DateMath.MILLISECONDS_IN_DAY).toInt()
        var time = utcTime % DateMath.MILLISECONDS_IN_DAY // (utcTime - (epochDay * DateMath.MILLISECONDS_IN_DAY))

        if (time < 0) {
            time = DateMath.MILLISECONDS_IN_DAY + time
            epochDay -= 1
        }
        ...
        dayOfMonth = dom
        var newtime = time
        hours = (newtime / DateMath.MILLISECONDS_IN_HOUR).toInt()
        newtime -= hours * DateMath.MILLISECONDS_IN_HOUR
        minutes = (newtime / DateMath.MILLISECONDS_IN_MINUTE).toInt()
        newtime -= minutes * DateMath.MILLISECONDS_IN_MINUTE
        seconds = (newtime / DateMath.MILLISECONDS_IN_SECOND).toInt()
        newtime -= seconds * DateMath.MILLISECONDS_IN_SECOND
