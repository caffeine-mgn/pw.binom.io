...
//    }
    ...
    override fun read(dest: ByteBuffer): Int {
        ...
        return ref { sourceCPointer, remaining ->
            dest.ref { destCPointer, destRemaining ->
                ...
                internal_memcpy(sourceCPointer, destCPointer, len.convert())
                position += len
                dest.position += len
                len
            }
    ...
    actual fun realloc(newSize: Int): ByteBuffer {
        ...
        ref0 { oldCPointer, oldDataSize ->
            new.ref0 { newCPointer, newDataSize ->
                internal_memcpy(oldCPointer, newCPointer, len.convert())
            }
        }
    ...
    actual fun toByteArray(): ByteArray {
        ...
        if (remaining > 0) {
            ref { ptr, remaining ->
                r.usePinned {
                    internal_memcpy(ptr, it.addressOf(0), remaining.convert())
                }
            }
        }
        return r
    ...
    actual fun write(data: ByteArray, offset: Int, length: Int): Int {
        ...
        data.usePinned { data ->
            ref0 { cPointer, dataSize ->
                internal_memcpy(data.addressOf(offset), cPointer + position, len.convert())
                position += len
            }
        }
    ...
    actual override fun compact() {
        ...
        if (remaining > 0) {
            val size = remaining
            ref0 { cPointer, dataSize ->
                internal_memcpy(cPointer + position, cPointer, size.convert())
                position = size
                limit = capacity
            }
        } else {
    ...
    actual fun subBuffer(index: Int, length: Int): ByteBuffer {
        ...
        ref0 { oldCPointer, oldDataSize ->
            newBytes.ref0 { newCPointer, newDataSize ->
                internal_memcpy(oldCPointer + index, newCPointer, length.convert())
            }
        }
    ...
    actual fun get(dest: ByteArray, offset: Int, length: Int): Int {
        ...
        return ref0 { cPointer, dataSize ->
            ...
            dest.usePinned { dest ->
                internal_memcpy(cPointer + position, dest.addressOf(0), l.convert())
            }
            l
        }
    }
    ...
    actual fun free() {
        ...
        val size = remaining
        if (size > 0) {
            internal_memcpy(native + position, native, size.convert())
            position = 0
            limit = size
        } else {
            position = 0
