...
import pw.binom.collections.SortedMap
...
object StringPrep {
    ...
    class CharClass {
        ...
        private val tupleStart = ArrayList<Int>()
        private val tupleCount = ArrayList<Int>()

        constructor(mappings: SortedMap<Int, Int>) {
            mappings.forEach { (start, count) ->
                // Coalesce overlapping ranges.

                // Coalesce overlapping ranges.
                if (tupleStart.size > 0) {
                    val prevIndex = tupleStart.size - 1
                    val prevStart = tupleStart[prevIndex]
                    val prevCount = tupleCount[prevIndex]
                    ...
                    if (prevStart + prevCount >= start) {
                        val endPos = start + count
                        val newCount = endPos - prevStart
                        tupleCount[prevIndex] = newCount
                        return@forEach
                    }
                }
        ...
        fun isCharInClass(c: Int): Boolean {
            ...
            var pos = tupleStart.binarySearch(c, comparator = { a, b -> a - b })
            if (pos >= 0) {
            ...
            // tupleStart[pos] is <= c.
            val start: Int = tupleStart[pos]
            val count: Int = tupleCount[pos]
            return start <= c && c < start + count
        }

        companion object {
            fun fromList(charMap: IntArray): CharClass {
                val mapping = TreeMap<Int, Int>()
                for (i in charMap.indices) mapping[charMap[i]] = 1
                return CharClass(mapping)
            }

            fun fromRanges(charMap: IntArray): CharClass {
                // There must be an even number of tuples in RANGES tables.
                if (charMap.size % 2 != 0) throw IllegalArgumentException("Invalid character list size")
                val mapping = TreeMap<Int, Int>()
                var i = 0
            ...
            fun fromClasses(vararg classes: CharClass): CharClass {
                val mapping = TreeMap<Int, Int>()
                for (charClass in classes) {
                    for (i in charClass.tupleStart.indices) {
                        val start: Int = charClass.tupleStart.get(i)
                        val count: Int = charClass.tupleCount.get(i)
                        mapping.put(start, count)
                    }
                }
                return CharClass(mapping)
    ...
    val saslProhibited = CharClass.fromClasses(C12, C21, C22, C3, C4, C5, C6, C7, C8, C9)

    /** A prohibited string has been passed to StringPrep.  */
    abstract class StringPrepError protected constructor(message: String?) : Exception(message)

    /** A prohibited character was detected.  */
    open class StringPrepProhibitedCharacter : StringPrepError {
        internal constructor() : super("String contains a prohibited character") {}
        protected constructor(s: String?) : super(s) {}
    }

    /** A prohibited unassigned codepoint was detected.  */
    class StringPrepUnassignedCodepoint internal constructor() :
        StringPrepProhibitedCharacter("String contains an unassigned codepoint")

    /** RTL verification has failed, according to rfc3454 section 6.  */
    open class StringPrepRTLError internal constructor() : StringPrepError("Invalid RTL string")

    class StringPrepRTLErrorBothRALandL : StringPrepRTLError()

    class StringPrepRTLErrorRALWithoutPrefix : StringPrepRTLError()

    class StringPrepRTLErrorRALWithoutSuffix : StringPrepRTLError()
    ...
    /** Replace each character of `s` which is in the [CharClass] mapFrom
     * with the string `mapTo`.  */
    fun applyMapTo(s: String, mapFrom: CharClass, mapTo: String?): String {
        val result = StringBuilder()
        var i = 0
        while (i < s.length) {
            val c = s[i].code
            val charCount: Int = charCount(c)
            if (mapFrom.isCharInClass(c)) result.append(mapTo) else result.append(s, i, i + charCount)
            i += charCount
        }
        return result.toString()
    }

    /** Return the first character index in s which is in [CharClass], or -1 if
     * no character is in the class. */
    fun containsCharacterInClass(s: String, charClass: CharClass): Int {
        var i = 0
        while (i < s.length) {
            val c: Int = s[i].code
            if (charClass.isCharInClass(c)) return i
            i += charCount(c)
        }
        return -1
    }
     ...
     * throw a subclass of [StringPrepRTLError].  */
    private fun verifyRTL(s: String) {
        val containsRAL = containsCharacterInClass(s, D1)
        if (containsRAL != -1) {
            // 2) If a string contains any RandALCat character, the string MUST NOT
            // contain any LCat character.
            val containsL = containsCharacterInClass(s, D2)
            if (containsL != -1) throw StringPrepRTLErrorBothRALandL()
            // 3) If a string contains any RandALCat character, a RandALCat
            // character MUST be the first character of the string
            if (containsRAL != 0) throw StringPrepRTLErrorRALWithoutPrefix()

            // ... and a RandALCat character MUST be the last character of the string.
            if (!D1.isCharInClass(s[s.length - 1].code)) throw StringPrepRTLErrorRALWithoutSuffix()
        }
    }

    /** Apply SASLPrep and return the result.  `` is treated as a stored string.  */
    @Throws(StringPrepError::class)
    fun prepAsStoredString(s: String): String? {
        var s = s
        ...
        // code points.
        val containsUnassignedCodepoint = containsCharacterInClass(s, A1)
        if (containsUnassignedCodepoint != -1) throw StringPrepUnassignedCodepoint()
        return s
    }

    /** Apply SASLPrep and return the result.  `` is treated as a query string.  */
    fun prepAsQueryString(s: String): String {
        // 1) Map
        // rfc4013: 2.1. Mapping
        ...
        // 3) Prohibit
        val idx = containsCharacterInClass(s, saslProhibited)
        if (idx != -1) throw StringPrepProhibitedCharacter()
    ...
    fun isContainingProhibitedCharacters(s: String): Boolean {
        val idx = containsCharacterInClass(s, saslProhibited)
        return idx != -1
    }
}
