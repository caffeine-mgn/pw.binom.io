 ...
 */
enum class ConnectionState(val value: Byte) {
    Outgoing(1 shl 1),
    Connected(1 shl 2),
    ShutdownRequested(1 shl 3),
    Disconnected(1 shl 4),
    EndPointChange(1 shl 5),
    Any(
        Outgoing.value or Connected.value or ShutdownRequested.value or EndPointChange.value,
    ),
}

internal enum class ConnectRequestResult {
    None, P2PLose, // when peer connecting
    Reconnection, // when peer was connected
    NewConnection, // when peer was disconnected
}

internal enum class DisconnectResult {
    None, Reject, Disconnect,
}

internal enum class ShutdownResult {
    None, Success, WasConnected,
}

class NetPeer {
    //Ping and RTT
    private var _rtt = 0;
    private var _avgRtt = 0;
    private var _rttCount = 0;
    private var _resendDelay = 27.0;
    private var _pingSendTimer = 0;
    private var _rttResetTimer = 0;
    private val _pingTimer: Stopwatch = Stopwatch();
    private var _timeSinceLastPacket = 0;
    private var _remoteDelta = 0L;

    //Common
    private val _shutdownLock = ReentrantLock();

    // TODO make volatile
    internal var NextPeer: NetPeer? = null
    internal var PrevPeer: NetPeer? = null

    internal var ConnectionNum: Byte
        get() = _connectNum
        set(value) {
            _connectNum = value;
    ...
    //Channels
    private val _unreliableChannel: Queue<NetPacket>
    private val _channelSendQueue: ConcurrentQueue<BaseChannel>
    private val _channels: Array<BaseChannel>;

    //MTU
    private var _mtu = 0;
    private var _mtuIdx = 0;
    private var _finishMtu = false;
    private var _mtuCheckTimer = 0;
    private var _mtuCheckAttempts = 0;
    private val MtuCheckDelay = 1000;
    private val MaxMtuCheckAttempts = 4;
    private val _mtuMutex = new

    object();

    //Fragment
    private class IncomingFragments {
        public var Fragments: Array<NetPacket>? = null
        public var ReceivedCount = 0
        public var TotalSize = 0
        public var ChannelId = 0.toByte()
    }

    private var _fragmentId = 0
    private val _holdedFragments: MutableMap<UShort, IncomingFragments>
    private val _deliveredFragments: MutableMap<UShort, UShort>

    //Merging
    private val _mergeData: NetPacket
    private var _mergePos = 0
    private var _mergeCount = 0

    //Connection
    private var _remoteEndPoint: NetworkAddress;
    private var _connectAttempts = 0;
    private var _connectTimer = 0;
    private var _connectTime = 0L;
    private var _connectNum = 0.toByte();
    private var _connectionState: ConnectionState;
    private var _shutdownPacket: NetPacket;
    private val ShutdownDelay = 300;
    private var _shutdownTimer = 0;
    private val _pingPacket: NetPacket;
    private val _pongPacket: NetPacket;
    private val _connectRequestPacket: NetPacket;
    private val _connectAcceptPacket: NetPacket;
    ...
    /// </summary>
    public val NetManager: NetManager;
    ...
    /// </summary>
    public val RemoteTimeDelta: Long
        get() = _remoteDelta

    /// <summary>
    /// Remote UTC time (not accurate)
    /// </summary>
    public val RemoteUtcTime: DateTime
        get() = DateTime(DateTime.nowTime + _remoteDelta)

    /// <summary>
    /// Time since last packet received (including internal library packets)
    /// </summary>
    public val TimeSinceLastPacket: Int
        get() = _timeSinceLastPacket

    internal val ResendDelay: Double
        get() = _resendDelay
    ...
    internal constructor(netManager: NetManager, remoteEndPoint: IPEndPoint, id: Int) {
        Id = id;
        ...
        _mergeData = NetPacket(PacketProperty.Merged, NetConstants.MaxPacketSize);
        _pongPacket = NetPacket(PacketProperty.Pong, 0);
        _pingPacket = NetPacket(PacketProperty.Ping, 0) { Sequence = 1 };

        _unreliableChannel = Queue<NetPacket>();
        _holdedFragments = HashMap<UShort, IncomingFragments>();
        _deliveredFragments = HashMap<UShort, UShort>();

        _channels = arrayOfNulls<BaseChannel>(netManager.ChannelsCount * NetConstants.ChannelTypeCount)
        _channelSendQueue = ConcurrentQueue<BaseChannel>();
    }

    internal fun InitiateEndPointChange() {
        ResetMtu();
        _connectionState = ConnectionState.EndPointChange;
    }

    internal fun FinishEndPointChange(newEndPoint: IPEndPoint) {
    ...
    public fun GetPacketsCountInReliableQueue(byte channelNumber, bool ordered): Int {
    ...
    public fun CreatePacketFromPool(DeliveryMethod deliveryMethod, byte channelNumber): PooledPacket {
    ...
    public fun SendPooledPacket(PooledPacket packet, int userDataSize) {
    ...
    private fun CreateChannel(idx:Byte): BaseChannel {
        val newChannel = _channels [idx];
        if (newChannel != null)
            return newChannel;
    ...
    internal constructor(netManager:NetManager, remoteEndPoint:NetworkAddress, id:Int, connectNum:Byte, connectData:NetDataWriter)
    : this(netManager, remoteEndPoint, id)
    {
        _connectTime = DateTime.nowTime
    ...
    internal constructor(netManager:NetManager, request:ConnectionRequest, id:Int) : this(netManager, request.RemoteEndPoint, id)
    {
    ...
    //Reject
    internal fun Reject(requestData:NetConnectRequestPacket, data:ByteArray , start:Int, length:Int)
    {
    ...
    internal fun ProcessConnectAccept(NetConnectAcceptPacket packet): Boolean {
    ...
    public fun GetMaxSinglePacketSize(options:DeliveryMethod): Int {
        val bb = if (options == DeliveryMethod.Unreliable) PacketProperty . Unreliable else PacketProperty . Channeled
        return _mtu - NetPacket.GetHeaderSize(bb)
    }
    ...
    /// </exception>
    public fun SendWithDeliveryEvent(data:ByteArray , channelNumber:Byte, deliveryMethod:DeliveryMethod,userData:Any?)
    {
        if (deliveryMethod != DeliveryMethod.ReliableOrdered && deliveryMethod != DeliveryMethod.ReliableUnordered)
            throw IllegalArgumentException ("Delivery event will work only for ReliableOrdered/Unordered packets");
        SendInternal(data, 0, data.size, channelNumber, deliveryMethod, userData);
    }
    ...
    ///     If you trying to send unreliable packet type<para/>
    /// </exception>
    public fun SendWithDeliveryEvent(
        byte
        [] data , int start, int length, byte channelNumber, DeliveryMethod deliveryMethod,
    object userData)
    {
        if (deliveryMethod != DeliveryMethod.ReliableOrdered && deliveryMethod != DeliveryMethod.ReliableUnordered)
    ...
    /// </exception>
    public fun Send(byte[] data , DeliveryMethod deliveryMethod)
    {
    ...
    ///     Fragment count exceeded ushort.MaxValue<para/>
    /// </exception>
    public fun Send(NetDataWriter dataWriter, DeliveryMethod deliveryMethod) {
    ...
    ///     Fragment count exceeded ushort.MaxValue<para/>
    /// </exception>
    public fun Send(byte[] data , int start, int length, DeliveryMethod options)
    {
    ...
    ///     Fragment count exceeded ushort.MaxValue<para/>
    /// </exception>
    public fun Send(byte[] data , byte channelNumber, DeliveryMethod deliveryMethod)
    {
    ...
    ///     Fragment count exceeded ushort.MaxValue<para/>
    /// </exception>
    public fun Send(NetDataWriter dataWriter, byte channelNumber, DeliveryMethod deliveryMethod) {
    ...
    ///     Fragment count exceeded ushort.MaxValue<para/>
    /// </exception>
    public fun Send(data:ReadOnlySpan<byte> , channelNumber:Byte, deliveryMethod:DeliveryMethod)
    {
    ...
    private fun SendInternal(
        ...
        if (_connectionState != ConnectionState.Connected || channelNumber >= _channels.size)
            return;
    ...
    public fun Disconnect(data:ByteArray )
    {
        NetManager.DisconnectPeer(this, data);
    }

    public fun Disconnect(writer:NetDataWriter)
    {
        NetManager.DisconnectPeer(this, writer);
    }

    public fun Disconnect(data:ByteArray , start:Int, count:Int)
    {
    ...
    internal fun ProcessDisconnect(packet:NetPacket):DisconnectResult
    {
        if ((_connectionState == ConnectionState.Connected || _connectionState == ConnectionState.Outgoing) &&
            ...
            return if (_connectionState == ConnectionState.Connected)
    ...
    internal fun AddToReliableChannelSendQueue(channel:BaseChannel) {
        _channelSendQueue.Enqueue(channel);
    }

    internal fun Shutdown(data:ByteArray , start:Int, length:Int, force:Boolean):ShutdownResult
    {
        lock(_shutdownLock)
        {
    ...
    private fun UpdateRoundTripTime(roundTripTime:Int) {
        _rtt += roundTripTime;
        _rttCount++;
    ...
    internal fun AddReliablePacket(DeliveryMethod method, NetPacket p) {
        if (p.IsFragmented) {
    ...
    private fun ProcessMtuPacket(NetPacket packet) {
        //header + int
    ...
    private fun UpdateMtuLogic(int deltaTime) {
        if (_finishMtu)
            return;
    ...
    internal fun ProcessConnectRequest(connRequest:NetConnectRequestPacket):ConnectRequestResult
    {
    ...
    internal fun ProcessPacket(NetPacket packet) {
        //not initialized
        ...
        switch(packet.Property)
        {
            ...
            if (packet.Sequence == _pingPacket.Sequence) {
                ...
                _remoteDelta = BitConverter.ToInt64(
                    packet.RawData,
                    3
            ...
            var channel =
                _channels[packet.ChannelId] ?? (packet.Property == PacketProperty.Ack ? null : CreateChannel(packet.ChannelId));
            if (channel != null) {
    ...
    private void SendMerged()
    {
        ...
        if (_mergeCount > 1) {
            ...
            bytesSent =
                NetManager.SendRaw(_mergeData.RawData, NetConstants.HeaderSize + 2, _mergePos - 2, _remoteEndPoint);
        }
    ...
    internal fun SendUserData(NetPacket packet) {
        packet.ConnectionNumber = _connectNum;
    ...
    internal fun RecycleAndDeliver(NetPacket packet) {
