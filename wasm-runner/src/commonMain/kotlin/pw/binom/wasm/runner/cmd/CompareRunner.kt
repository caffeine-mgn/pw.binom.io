package pw.binom.wasm.runner.cmd

import pw.binom.wasm.node.inst.Compare
import pw.binom.wasm.node.inst.Inst
import pw.binom.wasm.node.inst.Numeric
import pw.binom.wasm.runner.ArrayStack
import pw.binom.wasm.runner.MemorySpace

object CompareRunner {
  fun run(cmd: Compare, stack: ArrayStack, memory: List<MemorySpace>): Inst? {
    return when (cmd) {
      is Compare.F32_EQ -> TODO()
      is Compare.F32_GE -> TODO()
      is Compare.F32_GT -> TODO()
      is Compare.F32_LE -> TODO()
      is Compare.F32_LT -> TODO()
      is Compare.F32_NE -> TODO()
      is Compare.F64_EQ -> TODO()
      is Compare.F64_GE -> TODO()
      is Compare.F64_GT -> TODO()
      is Compare.F64_LE -> TODO()
      is Compare.F64_LT -> TODO()
      is Compare.F64_NE -> TODO()
      is Compare.I32_EQ -> {
        val a = stack.popI32()
        val b = stack.popI32()
        stack.pushI32(if (b == a) 1 else 0)
        cmd.next
      }
      is Compare.I32_EQZ -> {
        val a = stack.popI32()
        stack.pushI32(if (a == 0) 1 else 0)
        cmd.next
      }
      is Compare.I32_GE_S -> {
        val a = stack.popI32()
        val b = stack.popI32()
        stack.pushI32(if (b >= a) 1 else 0)
        cmd.next
      }
      is Compare.I32_GE_U -> {
        val a = stack.popI32().toUInt()
        val b = stack.popI32().toUInt()
        stack.pushI32(if (b >= a) 1 else 0)
        cmd.next
      }
      is Compare.I32_GT_S -> {
        val a = stack.popI32()
        val b = stack.popI32()
        stack.pushI32(if (b > a) 1 else 0)
        cmd.next
      }
      is Compare.I32_GT_U -> {
        val a = stack.popI32().toUInt()
        val b = stack.popI32().toUInt()
        stack.pushI32(if (b > a) 1 else 0)
        cmd.next
      }
      is Compare.I32_LE_S -> {
        val a = stack.popI32()
        val b = stack.popI32()
        stack.pushI32(if (b <= a) 1 else 0)
        cmd.next
      }
      is Compare.I32_LE_U -> {
        val a = stack.popI32().toUInt()
        val b = stack.popI32().toUInt()
        stack.pushI32(if (b <= a) 1 else 0)
        cmd.next
      }
      is Compare.I32_LT_S -> {
        val a = stack.popI32()
        val b = stack.popI32()
        stack.pushI32(if (b < a) 1 else 0)
        cmd.next
      }
      is Compare.I32_LT_U -> {
        val a = stack.popI32().toUInt()
        val b = stack.popI32().toUInt()
        stack.pushI32(if (b < a) 1 else 0)
        cmd.next
      }
      is Compare.I32_NE -> {
        val a = stack.popI32()
        val b = stack.popI32()
        stack.pushI32(if (b != a) 1 else 0)
        cmd.next
      }
      is Compare.I64_EQ -> {
        val a = stack.popI64()
        val b = stack.popI64()
        stack.pushI32(if (b == a) 1 else 0)
        cmd.next
      }
      is Compare.I64_EQZ -> {
        val a = stack.popI64()
        stack.pushI32(if (a == 0L) 1 else 0)
        cmd.next
      }
      is Compare.I64_GE_S -> {
        val a = stack.popI64()
        val b = stack.popI64()
        stack.pushI32(if (b >= a) 1 else 0)
        cmd.next
      }
      is Compare.I64_GE_U -> {
        val a = stack.popI64().toULong()
        val b = stack.popI64().toULong()
        stack.pushI32(if (b >= a) 1 else 0)
        cmd.next
      }
      is Compare.I64_GT_S -> {
        val a = stack.popI64()
        val b = stack.popI64()
        stack.pushI32(if (b > a) 1 else 0)
        cmd.next
      }
      is Compare.I64_GT_U -> TODO()
      is Compare.I64_LE_S -> TODO()
      is Compare.I64_LE_U -> TODO()
      is Compare.I64_LT_S -> TODO()
      is Compare.I64_LT_U -> TODO()
      is Compare.I64_NE -> {
        val a = stack.popI64()
        val b = stack.popI64()
        stack.pushI32(if (b != a) 1 else 0)
        cmd.next
      }
      is Compare.REF_EQ -> TODO()
    }
  }
}
