...
class NatsConnector3(
    ...
    private inner class MessageImpl : Message {
        override val connection: NatsConnector3
            get() = this@NatsConnector3
...
//        val reader: AsyncBufferedAsciiInputReader
                            ...
                            msg.startsWith("MSG ") -> {
                                ...
                                try {
                                    ...
                                    if (messageListener != null) {
                                        this@NatsConnector3.msg.subject = items[1]
                                        this@NatsConnector3.msg.sid = UUID.fromString(items[2])
                                        this@NatsConnector3.msg.replyTo = if (items.size == 5) {
                                            items[3]
                                        } else {
                                            null
                                        }
                                        data.flip()
                                        this@NatsConnector3.msg.data = data
                                        messageListener.invoke(this@NatsConnector3.msg)
                                    }
                                } finally {
                                    data.close()
                                }
    ...
    private var conns: NatsConnection? = null
...
//                reader = AsyncBufferedAsciiInputReader(connection)
    ...
    private class Subscribe(val subject: String, val group: String?)
    ...
    suspend fun subscribe(subject: String, group: String? = null): UUID {
        ...
        val s = Subscribe(subject = subject, group = group)
        subscribes[subscribeId] = s
