...
//                }
      ...
      con.prepareStatement("SELECT TIMESTAMP '2020-01-05 15:43:36.000000'").useAsync {
        try {
          it.executeQuery().useAsync {
            println(it.columns.joinToString(" | "))
            while (it.next()) {
      ...
      con.prepareStatement("select * from buyers limit 3").useAsync {
        try {
          it.executeQuery().useAsync {
            println(it.columns.joinToString(" | "))
            while (it.next()) {
  ...
  fun numArgTest() {
    pg {
      it.prepareStatement("select $1").useAsync {
  ...
  fun invalidPreparedStatement() {
    pg {
      try {
        it.prepareStatement("select * from ooooo").useAsync {
          it.executeUpdate()
        }
        fail()
...
(
	...
	constraint member_tag_pkey
      ...
      connect.prepareStatement("delete from member_tag where member_id=? and tag_id=?").useAsync { ps ->
        ps.set(0, 0)
        ps.set(1, 0)
//                repeat(30) {
        println("---===START===---")
...
//                }
  ...
  fun missingColumn() {
    pg {
      ...
      try {
        it.prepareStatement("select sum(amount) from missing_column_test where company_id=?").useAsync {
          it.executeQuery(10).useAsync {
          }
        }
        fail()
      } catch (e: PostgresqlException) {
  ...
  fun testString() {
    pg { con ->
      con.prepareStatement("""select 'Привет' """).executeQuery().useAsync {
        assertTrue(it.next())
      ...
      con.prepareStatement("""select '' """).executeQuery().useAsync {
        assertTrue(it.next())
...
//                )
      ...
      con.prepareStatement("insert into test1 (id,name) values (?,?)").useAsync {
        try {
      ...
      con.prepareStatement("insert into test1 (id,name) values (?,?) returning id").useAsync {
        it.set(0, Random.nextUuid())
        it.set(1, Random.nextUuid().toString())
