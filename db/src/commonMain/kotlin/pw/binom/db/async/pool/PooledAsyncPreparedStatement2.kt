...
class PooledAsyncPreparedStatement2(val pool: AsyncConnectionPoolImpl, val sql: String) : AsyncCloseable {
  ...
  suspend fun executeQuery(
    connection: PooledAsyncConnectionImpl,
    vararg args: Any?,
...
value class SelectQuery(val query: SQLQueryNamedArguments) {
  ...
  ) = connection.usePreparedStatement(query.sql).executeQuery(*query.buildArguments(*args)).useAsync {
    processing(it)
  }

  fun <T : Any> mapper(mapper: suspend (AsyncResultSet) -> T) = SelectQueryWithMapper(query, mapper)
}
...
value class UpdateQuery(val query: SQLQueryNamedArguments) {
  ...
  suspend fun execute(
    connection: PooledAsyncConnection,
    vararg args: Pair<String, Any?>,
  ): Long = connection.usePreparedStatement(query.sql).executeUpdate(*query.buildArguments(*args))
...
suspend fun <T : Any> PooledAsyncConnection.selectOneOrNull(
  ...
  usePreparedStatement(query.query.sql).executeQuery(*query.query.buildArguments(*args)).useAsync {
    if (it.next()) {
      query.mapper(it)
    } else {
      null
...
suspend fun <T : Any> PooledAsyncConnection.update(
  query: UpdateQueryWithParam<T>,
  value: T,
...
suspend fun <T> PooledAsyncConnection.execute(
  ...
  vararg args: Pair<String, Any?>,
  processing: suspend (AsyncResultSet) -> T,
): T = query.execute(this, *args) { processing(it) }
