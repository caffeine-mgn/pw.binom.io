...
internal object InternalProtocolUtils {
  ...
  private suspend fun unpackListAsync(
    size: Int,
    buf: ByteBuffer,
    input: AsyncInput,
  ): List<Any?> {
  ...
  suspend fun unpackString(
    size: Int,
    buf: ByteBuffer,
    input: AsyncInput,
  ): String = unpackBytes(size, buf, input).decodeToString()

  suspend fun unpackBytes(
    size: Int,
    buf: ByteBuffer,
    input: AsyncInput,
  ): ByteArray {
    ...
    return when (type) {
      ...
      MP_FIXEXT -> {
        ...
        return when {
          ...
          typeInt >= MP_FIXARRAY_INT && typeInt <= MP_FIXARRAY_INT + MAX_4BIT ->
            unpackListAsync(
              typeInt - MP_FIXARRAY_INT,
              buf,
              input,
            )
          typeInt.toUInt() >= MP_FIXMAP_INT.toUInt() && typeInt.toUInt() <= MP_FIXMAP_INT.toUInt() + MAX_4BIT.toUInt() ->
            unpackMap(
              typeInt - MP_FIXMAP_INT,
              buf,
              input,
            )
          typeInt >= MP_FIXSTR_INT && typeInt <= MP_FIXSTR_INT + MAX_5BIT ->
            unpackString(
