...
open class WebDavClient constructor(val client: HttpClient, val url: URL) :
  FileSystem {
     ...
     */
    override suspend fun <T> useUser(
      user: Any?,
      func: suspend () -> T,
    ): T {
      if (user == null) {
    ...
    override suspend fun mkdir(path: Path): FileSystem.Entity? {
      ...
      val responseCode = r.getResponse().useAsync { it.responseCode }
      if (responseCode == 405) {
        return null
      }
      if (responseCode == 401) {
    ...
    suspend fun getDir(
      ...
      val reader = resp.readText().useAsync { it.xmlTree() }
      resp.asyncClose()
    ...
    override suspend fun new(
      path: Path,
      writeAction: suspend (AsyncOutput) -> Unit,
    ): FileSystem.Entity {
      var size = 0L
      new(path).useAsync { fout ->
