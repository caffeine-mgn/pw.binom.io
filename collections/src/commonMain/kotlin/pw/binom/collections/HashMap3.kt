...
internal fun <E> Array<E>.resetRange(fromIndex: Int, toIndex: Int) {
  (this as Array<Any?>).fill(element = null, fromIndex = fromIndex, toIndex = toIndex)
}
...
class HashMap3<K, V> private constructor(
  ...
  override fun equals(other: Any?): Boolean =
    other === this || (other is Map<*, *>) && contentEquals(other)
...
//    }
  ...
  private fun ensureCapacity(capacity: Int) {
    require(capacity >= 0) { "capacity should be more or equals than 0" }
  ...
  override fun toString(): String {
    return "HashMap3(keysArray[${keysArray.size}]=${keysArray.contentToString()}, valuesArray[${valuesArray?.size}]=${valuesArray?.contentToString()}, presenceArray[${presenceArray.size}]=${presenceArray.contentToString()}, hashArray[${hashArray.size}]=${hashArray.contentToString()}, maxProbeDistance=$maxProbeDistance, length=$length, hashShift=$hashShift, _size=$_size, keysView=$keysView, valuesView=$valuesView, entriesView=$entriesView, isReadOnly=$isReadOnly, capacity=$capacity, hashSize=$hashSize)"
  }
  ...
  internal class KeysItr<K, V>(map: HashMap3<K, V>) : Itr<K, V>(map), MutableIterator<K> {
    override fun next(): K {
  ...
  internal class ValuesItr<K, V>(map: HashMap3<K, V>) : Itr<K, V>(map), MutableIterator<V> {
    override fun next(): V {
      if (index >= map.length) throw NoSuchElementException()
      lastIndex = index++
  ...
  internal class EntriesItr<K, V>(map: HashMap3<K, V>) :
    Itr<K, V>(map),
...
internal class HashMapKeys<E> internal constructor(
  private val backing: HashMap3<E, *>,
...
typealias LinkedHashMap<K, V> = HashMap3<K, V>
