...
abstract class AbstractLocalService : CrossService {
    ...
    class LocalMethod<T>(val name: String, val func: suspend (Map<String, Any?>) -> T) : CrossService.CrossMethod<T> {
    ...
    inner class Provider<T>(val func: suspend (Map<String, Any?>) -> T) : PropertyDelegateProvider<Any, LocalMethod<T>> {
